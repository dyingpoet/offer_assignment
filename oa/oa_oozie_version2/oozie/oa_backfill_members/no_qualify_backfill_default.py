...
#--  Script Name  : no_qualify_backfill_default.py
#--  Description  : This python script will perform  default backfill process for members without scores and generate member_sc_cpn_no_qualify_backfill file.
#--  Input Files  : member_sc_cpn_qualify, sc_coupon_cnt, memberTest , backfill_offer
...
#-------------------------------------------------------------
#opening backfill_offers file in read mode
#-------------------------------------------------------------

inputObj5 = subprocess.Popen(["hadoop","fs","-cat",sys.argv[5]],stdout=subprocess.PIPE)
for line in inputObj5.stdout:
        words = line.rstrip().split(' ')
		cpn = words[0]
        backfillList.append(cpn)

inputObj5.stdout.close()
