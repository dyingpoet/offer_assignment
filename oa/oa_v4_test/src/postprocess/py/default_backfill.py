...
with open(memberClusterFile,'r') as inputObj:
    for line in inputObj:
        ...
        if member in memberSCOne:
            ### cluster reco that couldn't be assigned as backfilling options, use an empty memberClusterFile when there is no cluster recos
            memberSCOne[member].extend(recoClusterMap[label])
            memberClusterMap[member] = label

#members got at least one offer
for member in memberOfferOne:
    label = memberClusterMap[member]
    #n = len(defaultBackfillMap[label])
    n = len(memberOffersOne[member])
    idx = 0
    for cpn in defaultBackfillMap[label]:
        sc = fiaCpnMap[cpn]
        if sc not in memberSCOne[member] and offerAvail[cpn] > 0 and cpn not in memberOfferOne[member] and cpn not in memberOfferOne[member]:
            print ' '.join([member,sc,cpn])
            idx += 1 
            offerAvail[cpn] -= 1
        if idx + n == offerUpLimit:
            break
    if idx + n < offerUpLimit:
