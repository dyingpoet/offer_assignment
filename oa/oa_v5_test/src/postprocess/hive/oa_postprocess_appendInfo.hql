...
(
    SELECT
        CAST(c.value_coupon_nbr AS STRING) AS value_coupon_nbr
        , 1 AS treatment_type    
        ...
        , CAST(d.system_item_nbr AS INT)
        --, d.customer_item_nbr --d.is_item_nbr
        , CAST(c.prvdr_coupon_nbr AS INT) -- the coupon_item_nbr is not available in Tables c or d
        , c.start_date
        , c.end_date
    FROM
